...
public class OldGameManager : MonoBehaviour
{
    ...
    IEnumerator playVideo(bool firstRun = true)
    {
        ...
        //Play first video
        Instantiate(videoPlayerList[videoIndex].gameObject);
        VideoPlayer vp = videoPlayerList[videoIndex];
        vp.Play();
        ...
        while (vp.isPlaying)
        {
            Debug.Log("Playing time: " + vp.time + " INDEX: " + videoIndex);

            //(Check if we have reached half way)
            if (!reachedHalfWay && vp.time >= (vp.clip.length / 2))
            {
